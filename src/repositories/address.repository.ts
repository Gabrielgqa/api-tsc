...
export class AddressRepository {
  ...
  async update(id: number, address: string, number: string, district: string, city: string, state: string, complement?: string) {
    return await this.knex('addresses').where({ id }).update({ address, number, district, city, state, complement });
  }
